...
### Phase 1: 基础设施
- [ ] **1.1** <任务描述>
  - **文件**: `path/to/file.go:27-35`（新建 / 修改）
  - **改动类型**: MODIFY | ADD | DELETE
  - **参考**: `path/to/similar_file.go`（说明参考原因，如"结构最近似的同类实现"）
  - **受影响调用方**:
    - `path/to/caller_a.go:106` — 需适配（说明具体改动）
    - `path/to/caller_b.go:78` — 无需改动
  - **关键函数/结构体**: `SymbolName`
  - **注意**: （关键约束、字段说明、边界情况、i18n 注意事项等）
...
- **§3 任务格式**：每个任务包含文件路径、参考文件、注意事项。新建文件必须给出参考的同类文件（"参考 src/components/TurnTableCmp/FormPrizePoolCmp 的结构"）。
